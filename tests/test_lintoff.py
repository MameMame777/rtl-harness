...
def test_added_lint_off_is_found(tmp_path: Path):
    root = _repo(tmp_path)
    (root / "rtl" / "a.sv").write_text("module a;\n// verilator lint_off WIDTH\nendmodule\n", encoding="utf-8")
...
def test_untracked_file_and_waiver_change(tmp_path: Path):
    root = _repo(tmp_path)
    (root / "rtl" / "b.sv").write_text("module b; // verilog_lint: waive line-length\nendmodule\n", encoding="utf-8")
    ...
    assert lintoff.scan(root, include_untracked=False) and all(f["file"] == "waivers.verible" for f in lintoff.scan(root, include_untracked=False))
