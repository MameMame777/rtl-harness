...
def test_new_list_close_locally(consumer: Path):
    ...
    (consumer / ".harness" / "last" / "lint.json").write_text(
        json.dumps(
            {
                "status": "fail",
                "lint": {
    ...
    r = ticket.new(
        ...
        kind="deviation",
        domain="bus",
        title="AI dropped the timescale",
        model="claude",
        attach_last=True,
        notes=r"prompt mentioned C:\secret\path",  # hygiene-ok: redaction fixture
        submit=False,
    )
    ...
    assert (
        len(rows) == 1
        and rows[0]["id"] == r["id"]
        and rows[0]["title"] == "AI dropped the timescale"
    )
...
def test_escalate_preview_without_yes(consumer: Path):
    ...
    cfg = load_config(consumer)
    r = ticket.new(
        cfg, kind="tool", domain="verif", title="run_sim timeout", notes="took 1000s", submit=False
    )
    p = ticket.escalate(cfg, r["id"], yes=False)
    assert (
        p["submitted"] is False and p["repo"] == "acme/rtl-harness" and "took 1000s" in p["preview"]
    )
