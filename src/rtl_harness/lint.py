...
_VERIBLE_RX = re.compile(
    r"^(?P<file>.+?):(?P<line>\d+):(?P<col>\d+)(?:-\d+)?:\s*(?P<msg>.*?)\s*(?:\[(?P<rule>[a-z0-9-]+)\])?\s*$"
)
# "%Warning-CODE: path:line:col: message"  /  "%Error: path:line:col: message"  (Verilator)
_VERILATOR_RX = re.compile(
    r"^%(?P<kind>Warning|Error)(?:-(?P<code>[A-Za-z0-9_]+))?:\s*(?:(?P<file>(?:[A-Za-z]:)?[^:\s][^:]*?):(?P<line>\d+):(?P<col>\d+):)?\s*(?P<msg>.*)$"
)
    ...
    for f in files:
        ...
        if p.is_dir():
            out.extend(
                ...
                if q.is_file() and q.suffix.lower() in (".sv", ".svh", ".v", ".vh")
            )
        elif p.is_file():
            out.append(p)
        else:
...
def _format(targets: list[Path], cfg: Config, fix: bool) -> tuple[list[str], list[Finding]]:
    ...
    for t in targets:
        r = run(
            [exe, f"--flagfile={_posix(flags)}", _posix(t)],
            cwd=cfg.root,
            env=env,
...
def _verible_lint(targets: list[Path], cfg: Config) -> list[Finding]:
    ...
    args = [
        exe,
        f"--rules_config={_posix(rules.verible_rules_file())}",
        "--lint_fatal=false",
        "--parse_fatal=false",
    ]
    ...
    for line in (r.stdout + "\n" + r.stderr).splitlines():
        ...
        rule = m.group("rule") or (
            "syntax-error" if "syntax error" in m.group("msg").lower() else "verible"
        )
        out.append(
            Finding(
                _relpath(cfg, m.group("file")),
                int(m.group("line")),
                int(m.group("col")),
                rule,
                m.group("msg"),
...
def _verilator_lint(targets: list[Path], cfg: Config) -> list[Finding]:
    ...
    for line in (r.stdout + "\n" + r.stderr).splitlines():
        ...
        if (
            not msg
            or msg.startswith("...")
            or "See https://verilator.org" in msg
            or msg.startswith("Exiting due to")
        ):
            continue
...
def run_lint(
    files: list[str] | None = None, *, fix: bool = False, cfg: Config | None = None
) -> dict:
    ...
    for f in findings:
        f.severity = (
            "error"
            if f.rule == "syntax-error" or f.rule == "ERROR"
            else rules.severity_for(sev_map, f.tool, f.rule)
        )
