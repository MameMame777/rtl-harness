...
ITEMS: list[tuple[str, str, str, bool]] = [
    ("tickets/README.md", "tickets/README.md", "local ticket store (rtl-harness ticket new)", False),
    ("tickets/.gitkeep", "tickets/gitkeep", "keeps tickets/ in git", False),
    (".github/ISSUE_TEMPLATE/rtl-bug.yml", "ISSUE_TEMPLATE/rtl-bug.yml", "issue form: RTL bug", True),
    (".github/ISSUE_TEMPLATE/ai-deviation.yml", "ISSUE_TEMPLATE/ai-deviation.yml", "issue form: AI deviation", True),
    (".github/ISSUE_TEMPLATE/rule-proposal.yml", "ISSUE_TEMPLATE/rule-proposal.yml", "issue form: rule proposal", True),
    (".github/ISSUE_TEMPLATE/tool-bug.yml", "ISSUE_TEMPLATE/tool-bug.yml", "issue form: harness tool bug", True),
    (".github/ISSUE_TEMPLATE/config.yml", "ISSUE_TEMPLATE/config.yml", "issue chooser config", True),
    (".github/labels.yml", "labels.yml", "kind:* / domain:* / harness labels", True),
    (".github/workflows/labels-sync.yml", "labels-sync.yml", "syncs labels.yml to the repository", True),
    (".github/pull_request_template.md", "pull_request_template.md", "PR checklist incl. security self-review", True),
]


def github_remote(root: Path) -> str | None:
    try:
        r = subprocess.run(["git", "remote", "get-url", "origin"], cwd=str(root), capture_output=True,
...
def _render(text: str, cfg: Config) -> str:
    ...
    return (text.replace("{{escalate_repo}}", cfg.escalate_repo or "<owner>/rtl-harness")
...
def apply(cfg: Config, missing: list[dict]) -> list[str]:
    ...
    for item in missing:
        ...
        dest.write_text(_render(src.read_text(encoding="utf-8"), cfg), encoding="utf-8", newline="\n")
